locals {
  ...
  name_prefix = format("%s-%s", var.context.project, local.name_suffix)
  # cluster_name = "${var.project}-${local.name_suffix}"
  ...
  # for cluster
  enabled_cluster_log_types          = distinct(coalescelist(var.cluster_enabled_log_types, ["api", "audit", "authenticator", "controllerManager", "scheduler"]))
  ...
  # for worker
  worker_default_iam_role_id         = concat(aws_iam_role.workers.*.id, [""])[0]
  ...
  workers_group_defaults_defaults            = {
    ...
    iam_role_id                              = "local.worker_default_iam_role_id" # A custom IAM role id. Incompatible with iam_instance_profile_name.  Literal local.default_iam_role_id will never be used but if iam_role_id is not set, the local.default_iam_role_id interpolation will be used.
