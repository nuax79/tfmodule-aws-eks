...
output "worker_security_group_id" {
  description = "Security group ID attached to the EKS Worker-Node. On 1.14 or later, this is the 'Additional security groups' in the EKS console."
  value       = local.create_worker_security_group ? local.worker_security_group_id : var.worker_security_group_id
}
...
output "cluster_iam_role_name" {
  ...
  value       = local.cluster_role_name # local.cluster_iam_role_name
}
...
output "oidc_role_arn" {
  description = "EKS OIDC arn to provide authentication for kubernetes resources can control to AWS Resources like ELB, EC2"
...
output "context_tags" {
  description = "Outputs context tag from EKS Bastion tagging map"
  value       = local.tags
}
